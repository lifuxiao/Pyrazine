...
import pandas as pd
from googlenet_none import GoogLeNet1
# Load the pretrained googlenet model
googlenet123 = GoogLeNet1()
...
googlenet123 = googlenet123.to(device)

# Loss and optimizer
criterion = nn.MSELoss()
optimizer = optim.Adam(googlenet123.parameters(), lr=0.0001)
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            loss = criterion(outputs, labels)
            val_loss += loss.item()
    #torch.save(googlenet123.state_dict(), 'googlenet123_trained_200_256X1024.pth')
    if (epoch + 1) % 50 == 0:
        torch.save(googlenet123.state_dict(), f'googlenet123_None_epoch_{epoch+1}.pth')
